...
const StarIcon: React.FC = () => (
    <svg className="w-full h-full p-1 text-gray-700 opacity-60" fill="currentColor" viewBox="0 0 20 20">
...
const Board: React.FC<BoardProps> = ({ pawns, onPawnClick }) => {
  ...
  const getCellStylingClass = (row: number, col: number): string => {
    ...
    if (row === 7 && col === 7) return 'bg-gray-800';

    // Player home bases
    if (row < 6 && col < 6) return PLAYER_COLORS_HEX.RED.light;
    if (row < 6 && col > 8) return PLAYER_COLORS_HEX.GREEN.light;
    if (row > 8 && col < 6) return PLAYER_COLORS_HEX.BLUE.light;
    if (row > 8 && col > 8) return PLAYER_COLORS_HEX.YELLOW.light;
    ...
    if (trackIndex !== -1) {
        if (Object.values(START_SPOTS).some((spot, i) => spot === trackIndex)) {
            ...
            return PLAYER_COLORS_HEX[colorKey].light;
        }
        return 'bg-white';
    }
  ...
  return (
    <div className="grid grid-cols-15 grid-rows-15 w-full h-full aspect-square shadow-2xl border-4 border-gray-700 bg-gray-300">
      {cells.map(i => {
        ...
        return (
          <div key={i} className={`${cellBgClass} border border-gray-400/50 relative flex items-center justify-center`}>
